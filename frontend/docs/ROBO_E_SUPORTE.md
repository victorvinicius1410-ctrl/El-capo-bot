...
- **2026-08-18 ~18:45 UTC (BullEx comprou, overlay falhou)**
  - Sintoma: runtime `ORDER_SEND_FAILED` / `TEMPORARY_UNAVAILABLE`;
    `bullex-service` com `[REAL BUY SUCCESS]` no mesmo instante.
  - Correção: pool HTTP só da compra + timeout 45s; fora do semáforo dos candles.
- **2026-08-18 ~18:20 UTC (analisa, acha sinal, não compra)**
  - Sintoma: após o recycle das 17:15, cache ok, 0 compras. `ACTIVE_OK`
    seguido de `ROBOT_CYCLE_TIMEOUT`; buy-real 5s →
