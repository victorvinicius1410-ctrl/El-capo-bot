...
| Peça | Comportamento |
|------|----------------|
| `AppShell.resetScore` | `applyRobotMutationToCache` com `allowBlankOverwrite: true`; refetch em background (`void`) |
| `applyRobotMutationToCache` | Start/stop preservam placar vivo; só reset-score zera (`PLACAR_OVERLAY.md`) |
| `preserveRobotSessionScore` | Snapshot atrasado com placar cheio **não** desfaz 0-0 se `stop_reset_at` do painel for mais novo (2026-08-15) |

## Correção (2026-08-15) — reset “não pega” e placar cai no stop

Ver `PLACAR_OVERLAY.md`. Resumo:

- Runtime **não** re-hidrata no `stop` (evita 10x12 → 9x12).
- Front compara `stop_reset_at` para ignorar Redis/WS atrasado após reset.
