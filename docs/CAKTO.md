...
- **2026-08-24** — Compras como a do lead Acleudisson (conta criada, assinatura
  inativa): `purchase_approved` podia retornar 422 **depois** de criar Auth/perfil
  (falha no ledger). `subscription_created` subsequente não libera acesso. Correção:
  validação monetária antes do provisionamento; `grant_access` aplicado imediatamente
  após criar perfil; reprocessamento idempotente reativa cliente pendente.
