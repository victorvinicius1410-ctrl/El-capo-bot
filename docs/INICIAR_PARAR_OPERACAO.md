...
| Peça | Comportamento |
|------|----------------|
| `publish_robot_control_snapshot` | Grava Redis + agenda WS push (como o disconnect). Em mode=external **adota** o placar vivo do Redis/persistência se a memória do gateway estiver 0-0 (`adopt_live_session_score_if_blank`, 2026-08-21) — evita start/stop apagarem WIN/LOSS |
...
| `preserveRobotSessionScore` | Start/stop/WS/HTTP **não** trocam um placar vivo por 0-0. Ver `PLACAR_OVERLAY.md` |
