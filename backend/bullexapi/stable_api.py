...
class Bullex:
    ...
    def __init__(self, email, password, active_account_type="PRACTICE", proxies=None):
        ...
        # Dict no formato requests: {"http": "http://...", "https": "http://..."}.
        # Sem isso o login HTTP sai sempre pelo IP do VPS e cai em rate limit.
    ...
    def get_balances(self, timeout_seconds: float = 5.0):
        """
        Busca balances frescos no websocket.

        Args:
            timeout_seconds: Tempo máximo de espera pela resposta WS.

        Returns:
            Payload bruto com chave ``msg`` ou ``None`` se estourar o timeout.

        Note:
            O busy-wait sem teto anterior segurava o lock global do
            bullex-service sob carga e cascateava ``REAL_BALANCE_NOT_DETECTED``.
        """
        ...
        while self.api.balances_raw is None:
            if time.time() >= deadline:
                logging.error("**error** get_balances timeout after %ss", timeout_seconds)
                return None
            time.sleep(0.05)
