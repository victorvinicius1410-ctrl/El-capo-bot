...
from bullexapi.global_value import bind_api_session_globals, reset_session_globals
...
class WebsocketClient(object):
    ...
    def _ws_bind_globals(self):
        """Ativa SessionGlobals desta API no ContextVar (thread WS).

        Returns:
            Token para ``reset_session_globals``, ou None se bind falhar.
        """
        try:
            return bind_api_session_globals(self.api)
        except Exception:
            logging.getLogger(__name__).exception(
                "falha ao bind SessionGlobals no websocket"
            )
            return None

    @staticmethod
    def _ws_unbind_globals(token) -> None:
        if token is not None:
            reset_session_globals(token)

    def on_message(self, wss, message):  # pylint: disable=unused-argument
        """Method to process websocket messages."""
        token = self._ws_bind_globals()
        try:
    ...
    def on_close(self, *args, **kwargs):
        ...
        Accepts variable args because websocket-client versions differ
        (some pass ws, code, msg; older ones only code/msg).
        """
