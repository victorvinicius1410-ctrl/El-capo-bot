...
3. Aplique o mesmo schema de produção (SQL do bootstrap / migrations usadas no prod).
   Referência no repo: `backend/backend/el_capo_full_bootstrap.sql` e migrations em `docs/`.
4. Edite `/opt/elcapo2/backend/.env` e substitua os placeholders.
5. Rode: `/opt/elcapo2/scripts/deploy-backend.sh`
6. Crie um usuário admin de teste no Auth do projeto staging (não use contas reais de clientes).
